...
class CoDAGQAL:
    ...
    updated every forward() call, and is injected back into the attention
    computation via _attend_with_landmarks() so subsequent calls can attend
    beyond the current context window. backward() deliberately recomputes
    the forward pass WITHOUT the landmark augmentation: the cache is treated
    as a non-differentiable memory read (the same convention used for
    ordinary KV caches), which also sidesteps the fact that self.landmark_K/V
    have already been mutated (by this same forward() call) by the time
    backward() runs, so replaying the exact pre-update cache state is not
    possible without additional activation caching. This makes the returned
    gradient an approximation whenever landmark_count > 0.
    """
    ...
    def backward(self, x: np.ndarray, grad_out: np.ndarray,
        ...
        mask = np.triu(np.full((seq, seq), -1e9), k=1)
        a1 = self._softmax((Q_t @ K1_t) * self.scale + mask)  # (B, H, S, S)
        a2 = self._softmax((Q_t @ K2_t) * self.scale + mask)
        diff = a1 - self.lambda_param * a2

        O_t = diff @ V_t                                       # (B, H, S, dh)
        ...
        # Gradient through O_t = diff @ V_t
        ddiff = dO_t @ V_t.transpose(0, 1, 3, 2)              # (B, H, S, S)
        dV_t  = diff.transpose(0, 1, 3, 2) @ dO_t             # (B, H, S, dh)
        ...
        ds1 = self._softmax_backward(da1, a1) * self.scale     # (B, H, S, S)
        ds2 = self._softmax_backward(da2, a2) * self.scale

        # Gradient through s1 = Q_t @ K1_t
        dQ_t  = (ds1 @ K1_t.transpose(0, 1, 3, 2)             # from K1
                 + ds2 @ K2_t.transpose(0, 1, 3, 2))           # from K2
        dK1_t = Q_t.transpose(0, 1, 3, 2) @ ds1               # (B, H, dh, S)
        dK2_t = Q_t.transpose(0, 1, 3, 2) @ ds2
...
class MoELayer:
    ...
    def backward(self, x: np.ndarray,
        ...
        # Precompute token→expert routing map: O(n_tok * top_k).
        # tok_for_expert[e] = list of (token_idx, weight_idx_in_top_k)
        ...
        # dx accumulator: starts with the residual skip-connection gradient.
        dx_acc = gf.copy()
        ...
        for e in range(self.n_experts):
            ...
            w_arr   = np.array([float(wts[t, k]) for t, k in entries])  # (n_sel,)
            bx      = xf[sel_tok]                                         # (n_sel, d)

            # Scale upstream gradient by routing weight before passing to expert.
            weighted_gf = gf[sel_tok] * w_arr[:, np.newaxis]

            d_x, grads = self.experts[e].backward(bx, weighted_gf)

            # d_x already carries the w_arr factor (FFNBlock.backward is linear
            # in grad_out, and weighted_gf = gf * w_arr was passed as grad_out),
            # so accumulate it directly. Multiplying by w_arr again here would
            # double-apply the routing weight and shrink upstream gradients by
            # a spurious factor of w_e.
...
class MetacognitiveTrainingLoop:
    ...
    def step_one(self) -> Dict:
        ...
        # Use current step as a deterministic seq_offset so training is
        # reproducible and we never draw from the engine's internal PRNG here.
        # The offset must leave room for a full seq_len window before max_seq,
        # otherwise RoPE.rotate()/rotate_inverse() raise once seq_off + seq_len
        # exceeds the position table (this previously crashed any run longer
        # than max_seq - seq_len steps).
        offset_span = max(1, cfg.max_seq - cfg.seq_len + 1)
        seq_off = self.step % offset_span
